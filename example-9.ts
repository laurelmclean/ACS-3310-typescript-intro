...
import { KaijuType } from "./example-8"
import { Kaiju } from "./example-8"
...
function rampage(kaiju: Kaiju, city: string): string {
	...
	let result: string = `${name} ${action} over ${city} causing ${power * 10000} damage!`
	return result
}

const gojira = new Kaiju('Gojira', KaijuType.lizard, 90)
// Define two more monsters
const kingKong = new Kaiju("King Kong", KaijuType.ape, 70);
const rodan = new Kaiju("Rodan", KaijuType.flying, 60);
